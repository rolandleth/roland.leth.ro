...
Unlike posts, a future-dated guide is **not** scheduled — there's no read-time `datetime <= now` machinery here, so it goes live immediately with a future `datePublished`. Don't future-date a guide unless that's what you want.
