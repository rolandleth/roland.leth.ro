...
| `links[].url` | **Ask** (App Store: app id) | Real `https` URL. For the App Store, ask for the numeric **app id** and build `https://apps.apple.com/app/id<id>` (one id covers iPhone/iPad/Mac). Get website/GitHub URLs from the user. Stage a single App Store link when one id covers every platform: for an own app, a second App Store link (a separate Mac listing, say) replaces the badge with pills. If the app really has two listings, tell the user that trade-off before staging both. |
