...
describe("AdminNav — handleRevalidate", () => {
	it("shows the revalidated scope on success", async () => {
		mockRouter()
		global.fetch = vi.fn().mockResolvedValue({
			ok: true,
			status: 200,
			json: () => Promise.resolve({ ok: true, sections: ["tech", "life"] }),
		})

		render(<AdminNav />)
		await user.click(screen.getByRole("button", { name: /revalidate caches/i }))

		await waitFor(() =>
			expect(
				screen.getByText(/posts \(tech, life\) \+ projects/)
			).toBeInTheDocument()
		)
	})

	it("surfaces an error and stays retriable on a non-ok response", async () => {
		mockRouter()
		global.fetch = vi.fn().mockResolvedValue({
			ok: false,
			status: 500,
		...
		render(<AdminNav />)
		const button = screen.getByRole("button", { name: /revalidate caches/i })
		await user.click(button)
