...
import {
	...
	buildScheduledPostMarkdownFile,
} from "@/lib/content/postMarkdown"
...
// guards (section validation + `loadPostResolution`'s published + `datetime <=
// now` filtering) so the `.md` view can never expose a post the HTML page hides.
// A miss is a plain 404: the legacy-slug alias redirect is an HTML-only concern
...
// - `fetchPostRow`'s `post-{section}-{slug}` and `post-pages` tags ride up onto
...
export async function GET(
	...
	// (a markdown body has nowhere to carry robots meta). Prerendered under the
	// same tags via the shared cached row, so the same cron bust replaces it with
	// the real markdown.
	if (resolved.status === "scheduled") {
		const body = buildScheduledPostMarkdownFile(
			resolved.scheduled,
			{ section, slug },
			getSiteUrl()
		)
	...
	const body = buildPostMarkdownFile(resolved.post, getSiteUrl())
